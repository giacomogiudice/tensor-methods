...
\]
Show that these states are eigenstates of $H$ and have an energy contribution $+2$ relative to the ground state.
As a matter of fact, they are the first excited states.
\note{Note} Notice how the energy constribution does not depend on the length of the path.
\end{section}
...
The toric code can be generalized to planar graphs, since we can still define $A_v$ and $B_p$, while preserving $[A_v,B_p] = 0$.
Additionally, we should keep the vertices at the edges \emph{open}, i.e. they don't have a corresponding $A_v$ term (this is also called rough boundary conditions). Do you see why?
Predict the number of ground states of the toric code with two punctures.
\begin{figure}[h]
